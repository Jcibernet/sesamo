...
func TestLoad_ProjectSlug(t *testing.T) {
	...
	t.Run("custom slug accepted", func(t *testing.T) {
		applyEnv(t, base)
		t.Setenv("SESAMO_PROJECT_SLUG", "acme-prod")
		cfg, err := Load()
		if err != nil {
			t.Fatalf("Load() unexpected error: %v", err)
		}
		if cfg.ProjectSlug != "acme-prod" {
			t.Errorf("ProjectSlug = %q, want %q", cfg.ProjectSlug, "acme-prod")
		}
	})
	for _, bad := range []string{"Acme", "-prod", "prod-", "a b", "año"} {
